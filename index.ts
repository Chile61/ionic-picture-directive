...
export class Ionic2PictureDirective {
  ...
  @HostListener('click', ['$event.target']) handleKeyboardEvent(kbdEvent: KeyboardEvent){
    ...
    actionSheetPic.present();
      
  }

  _getpic(index){
    var arrayPic = [this.camera.PictureSourceType.PHOTOLIBRARY,this.camera.PictureSourceType.CAMERA];
    ...
    if(/localhost/.test(document.location.host)) {
      this.retornPicture.emit(this.foto);
    }

    this.camera.getPicture(options).then((imageData) => {
      let base64Image = 'data:image/jpeg;base64,' + imageData;
      this.retornPicture.emit(base64Image);
    }, (err) => {
      console.log(err);
    });
  }
